---
description: Webhookを利用してどのようにIntegrationで収集したデータを受けて来ることができていることを知ってみましょう。
---

# イベントを取得する\(Webhook\)

## Webhookとは? 

WebhookはIntegrationでブロックチェーンのデータを収集したときにデータをHTTP呼び出し方式で配信する概念です。Webhook方法でデータを受け取るためにサーバーのURLを提供する必要があります。Henesisでブロックチェーンデータが収集されたときされたURLにHTTP呼び出しをします。継続的にデータをポーリング（Polling）より新たにデータが生成されたときにデータをプッシュ（Push）するWebhook方式がリソースの使用効率的です。

## Ngrokの設定

{% hint style="info" %}
**What is ngrok?** 

ngrokは簡単外部に露出することができるURLを作ってくれるサービスです。そのURLに入ってきたトラフィックを内部ネットワークに転送することができます。固定IPがないローカル環境で簡単にwebhookが正常に動作することを確認時に役立ちます。
{% endhint %}

* [ngrok](https://ngrok.com/download)をダウンロードします。 
* （Linux）次のコマンドを使用してngrok解凍します。

```bash
$ unzip /path/to/ngrok.zip
```

* [ngrokで会員登録](https://dashboard.ngrok.com/signup)後、認証します。

```bash
$ ./ngrok authtoken <YOUR_AUTH_TOKEN>
```

* ローカルホスト（localhost）の3000ポートをExternal IPに露出させます。

```bash
$ ./ngrok http 3000
```

* 次のような画面が出力されることを確認します。 画面に表示さhttp://xxxx.ngrok.ioをクリップボードにコピーします。 

```bash
ngrok by @inconshreveable                                                                                                                                                             (Ctrl+C to quit)

Session Status                online
Account                        (Plan: Free)
Version                       2.3.29
Region                        United States (us)
Web Interface                 http://127.0.0.1:4040
Forwarding                    http://xxxx.ngrok.io -> http://localhost:3000
Forwarding                    https://xxxx.ngrok.io -> http://localhost:3000

Connections                   ttl     opn     rt1     rt5     p50     p90
                              0       0       0.00    0.00    0.00    0.00
```

## サーバーの実装

[Express](https://expressjs.com/)を利用してNodeベースのWebhookサーバーを以下のように作成します。

```bash
$ npm install express --save
```

{% code title="server.js" %}
```javascript
const express = require('express');
const app = express();
const port = 3000;

app.use(express.json());
app.post('/webhook', (req, res) => {
    console.log(JSON.stringify(req.body, null, '  '));
    res.send('Received')
});
app.listen(port, () => {
    console.log(`Application listening on port ${port}!`)
});
```
{% endcode %}

 `server.js`を利用してHenesisがHTTP POSTで呼び出すこと`/Webhook` URLを提供することができます。

## henesis.yaml設定と配布

`henesis.yaml`の詳細については、[Deploy Integration](https://docs.henesis.io/v/ko/subscribing-events/deploy-integration)章を参照してください。ここwebhookのためにセッティングする情報のみをもう一度見てみましょう。

{% code title="henesis.yaml" %}
```yaml
...
provider:
  type: webhook
  url: http://xxxx.ngrok.io/api/webhook
  method: POST
  headers: 
    Authorization: 'Bearer YOUR-OWN-TOKEN'
```
{% endcode %}

* `type`を`Webhook`に設定してください。 
* `Webhook`を受ける`url`と`method`を設定してください。 
* HTTP headerが必要な場合`headers`にご記入ください。（ex。Authorization）

作成したhenesis.yamlをCLIを介して配布してください。

## Running the Server

先に作成したserver.jsにNodeサーバーを実行してみます。

```bash
$ node server.js
```

次に、コンソールウィンドウで、ブロックチェーンメッセージが正しく出力されることを確認します。 

{% hint style="danger" %}
Duplicated data can be delivered if the connection is lost due to network anomalies \(such as Wi-Fi problems\). Therefore, the index \(message ID or block number\) for the received data must be stored to handle duplicated data.
{% endhint %}

Webhookにサブスクライブするメッセージのフォーマットは以下の通りです。

```javascript
{
  "events": [
    {
      "contractAddress": "0x20a1cff753ae36988f73507a5497f02369970d70",
      "contractName": "KittyCore",
      "eventName": "Transfer(address,address,uint256)",
      "data": [
        {
          "name": "from",
          "value": "0x1A96cDf0Beb4DcD61c0Fd2D5d0E852DF9E631b69",
          "type": "address"
        },
        {
          "name": "to",
          "value": "0xC1C88A7B90062866F54320b18Ee5DA65F79202D6",
          "type": "address"
        },
        {
          "name": "tokenId",
          "value": "130",
          "type": "uint256"
        }
      ],
      "transaction": {
        "blockHash": "0x6267f7e3db589571ca25ac6dfc8dd78db85215c722033dd129ee001d3d87d0b2",
        "blockNumber": "0x890ba8",
        "from": "0x1a96cdf0beb4dcd61c0fd2d5d0e852df9e631b69",
        "gas": "0x771000",
        "gasPrice": "0x5d21dba00",
        "hash": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
        "input": "",
        "nonce": "0x4f3",
        "senderTxHash": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
        "signatures": [
          {
            "V": "0x7f5",
            "R": "0xd63bf59c294bfd26e0f756b374af7c2b09e89a1b75bbe16d8a8999b96c97271d",
            "S": "0x64dae73afc0992b87a0a23275c00f8898681c621c6203d6a36fa427af3b0ca7e"
          }
        ],
        "to": "0x20a1cff753ae36988f73507a5497f02369970d70",
        "transactionIndex": "0x0",
        "type": "TxTypeLegacyTransaction",
        "typeInt": 0,
        "value": "0x0"
      }
    },
    {
      "contractAddress": "0xc1c88a7b90062866f54320b18ee5da65f79202d6",
      "contractName": "SiringClockAuction",
      "eventName": "AuctionCreated(uint256,uint256,uint256,uint256)",
      "data": [
        {
          "name": "tokenId",
          "value": "130",
          "type": "uint256"
        },
        {
          "name": "startingPrice",
          "value": "100000",
          "type": "uint256"
        },
        {
          "name": "endingPrice",
          "value": "100001",
          "type": "uint256"
        },
        {
          "name": "duration",
          "value": "100",
          "type": "uint256"
        }
      ],
      "transaction": {
        "blockHash": "0x6267f7e3db589571ca25ac6dfc8dd78db85215c722033dd129ee001d3d87d0b2",
        "blockNumber": "0x890ba8",
        "from": "0x1a96cdf0beb4dcd61c0fd2d5d0e852df9e631b69",
        "gas": "0x771000",
        "gasPrice": "0x5d21dba00",
        "hash": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
        "input": "",
        "nonce": "0x4f3",
        "senderTxHash": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
        "signatures": [
          {
            "V": "0x7f5",
            "R": "0xd63bf59c294bfd26e0f756b374af7c2b09e89a1b75bbe16d8a8999b96c97271d",
            "S": "0x64dae73afc0992b87a0a23275c00f8898681c621c6203d6a36fa427af3b0ca7e"
          }
        ],
        "to": "0x20a1cff753ae36988f73507a5497f02369970d70",
        "transactionIndex": "0x0",
        "type": "TxTypeLegacyTransaction",
        "typeInt": 0,
        "value": "0x0"
      }
    }
  ],
  "blockMeta": {
    "platform": "klaytn",
    "network": "baobab",
    "blockNumber": 8981416,
    "blockHeader": {
      "blockscore": "0x1",
      "extraData": "",
      "gasUsed": 140703,
      "governanceData": "0x",
      "hash": "0x6267f7e3db589571ca25ac6dfc8dd78db85215c722033dd129ee001d3d87d0b2",
      "logsBloom": "",
      "number": 8981416,
      "parentHash": "0xdb469243e1a98501c8bdf5278c9bd7c55df12fc057c561ca41f04137015d7cb6",
      "receiptsRoot": "0x82359dd8426d89949c393037c6056743401b8e1a48de6971ecc02ee2882e2e62",
      "reward": "0xa86fd667c6a340c53cc5d796ba84dbe1f29cb2f7",
      "size": 1071,
      "stateRoot": "0x87dc27fe65791184493bf4032155c32c20bae00eb79ac269ae1141ee23b135bf",
      "timestamp": 1570526845,
      "timestampFoS": "0xc",
      "totalBlockScore": "0x890ba9",
      "transactions": [
        "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d"
      ],
      "transactionsRoot": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
      "voteData": "0x"
    }
  },
  "userMeta": {
    "integrationId": "kitty-tutorial-klaytn-uijkh",
    "userId": 1
  }
}
```

