---
description: それでは実際にHenesis SDKを利用して、どのようにIntegrationからイベントを取得することができるか学んでいきましょう。
---

# イベントを取得する\(WebSocket\)

## henesis.yaml設定と配布

`henesis.yaml`の詳細については、[Deploy Integration](https://docs.henesis.io/v/ko/subscribing-events/deploy-integration)章を参照してください。ここwebSocketのためにセッティングする情報のみを再掲します。

{% code title="henesis.yaml" %}
```yaml
...
provider:
  type: webSocket 
  timeout: 10000
```
{% endcode %}

* `type`を`WebSocket`に設定してください。 
* `timeout`をデフォルトの`10000`に設定してください。（単位：ms）

作成した`henesis.yaml`をCLIを介して配布してください。

## Henesis SDK

HenesisでwebSocketを介してブロックチェーンデータを簡単に取得できるようSDKを提供しています。現在Javascript言語をサポートしており、将来的には他の言語もサポートする予定です。 インストール方法は、[リンク](https://docs.henesis.io/v/ko/installation/henesis-sdk)を参照してください。

## 認証

Henesis SDKを利用するためには、まず、`client-id`が必要です。`client-id`はユーザーごとに一意に付与された情報であり、以下のようにHenesis CLIを利用して確認できます。

```bash
$ henesis account:describe

Email: haechi@haechi.io
Name: haechi
Organization: haechi-labs
clientId: <client-id>
```

SDKでは以下のように`client-id`を使用して、データのサブスクリプションのためのHenesisインスタンスを生成することができます。

```javascript
import Henesis from '@haechi-labs/henesis-sdk-js'

const henesis = new Henesis('client-id');
```

## 取得

新しく作成したインスタンスの`subscribe`メソッドを利用して、`subscription`を作成することができます。

```javascript
// const subscription = await henesis.subscribe(topic: String, options: Object)

const subscription = await henesis.subscribe(
  "streamedBlock",
  {
    integrationId: "your-integration-id",
    subscriptionId: "your-subscription-id"
  }
);
```

* `topic` : 取得するトピックです。ブロック単位でイベントを取得する時は`streamedBlock`と入力します。つまり、特定のブロックで取得したいコントラクトのイベントが複数個ある場合、SDKはそれらを一度に配列として転送します。
* `options` : `subscription`を生成するために必要な情報
  * `integrationId`:  取得するIntegrationのIDです。
  * `subscriptionId`:  Henesisは`subscription`生成時点からブロックチェーンのデータを漏れなく転送します。たとえネットワークの状況などにより接続が切断がされても、再接続された時、最後に送信が成功されたデータの後から再送信します。`subscriptionId`は、ユーザが指定することができます。もし同じ`subscriptionId`が存在しない場合、新しい`subscription`が生成されます。

{% hint style="danger" %}
一本の`subscriptionId`に接続可能なクライアントの数は1個です。したがって、複数のクライアントが同じ`subscriptionId`に接続することができません。複数のクライアントからのデータのサブスクリプションが必要な場合、複数の`subscriptionId`を用いて複数の`subscription`を作成する必要があります。
{% endhint %}

上記の生成された`subscription`を使用して、以下のようにイベントを取得することができます。

```javascript
subscription.on('message', async (message) => {
  const events = message.data;
  // processing events
  // For example, you can save events to your database.
  message.ack(); // (MUST) Send an ACK message!!
});

subscription.on('error', err => {
  console.error(err);
});

subscription.on('close'. err => {
  console.error(err);
});
```

Henesisから渡されるデータを取得するために`on`関数を使用します。`subscription＃on（）`には、3つのトピックが存在します。各トピックのデータは、以下のような状況で発生します。各トピックで配信されるデータを利用して必要なロジックを実装することができます。

| Topic | 発生状況 |
| :--- | :--- |
| message | Henesisがブロックチェーンからデータを収集したとき |
| error | Henesisに障害状況が発生したとき（例：ネットワークエラー, ノードエラー） |
| close | Subscriptionが切断された時 |

### At Least Once Delivery <a id="reliability"></a>

If the connection to the server is successful through the SDK, Henesis guarantees that the data is delivered under any circumstances.However, duplicated data can be delivered if the connection is lost due to network anomalies \(such as Wi-Fi problems\), or if an ACK message is sent to Henesis but the ACK message does not reach the server. Therefore, the index \(message ID or block number\) for the received data must be stored to handle duplicated data.

{% hint style="danger" %}
必ず`message.ack（）`を利用して、ACKメッセージをHenesisに渡す必要があります。ACKメッセージが送信されない場合、Henesisはメッセージ配信に失敗したと判断して、次のデータを送信せず、同じデータを再送信します。
{% endhint %}

## Example of Response Message

`message`トピックで購読するデータのフォーマットは以下の通りです。

```javascript
{
  "events": [
    {
      "contractAddress": "0x20a1cff753ae36988f73507a5497f02369970d70",
      "contractName": "KittyCore",
      "eventName": "Transfer(address,address,uint256)",
      "data": [
        {
          "name": "from",
          "value": "0x1A96cDf0Beb4DcD61c0Fd2D5d0E852DF9E631b69",
          "type": "address"
        },
        {
          "name": "to",
          "value": "0xC1C88A7B90062866F54320b18Ee5DA65F79202D6",
          "type": "address"
        },
        {
          "name": "tokenId",
          "value": "130",
          "type": "uint256"
        }
      ],
      "transaction": {
        "blockHash": "0x6267f7e3db589571ca25ac6dfc8dd78db85215c722033dd129ee001d3d87d0b2",
        "blockNumber": "0x890ba8",
        "from": "0x1a96cdf0beb4dcd61c0fd2d5d0e852df9e631b69",
        "gas": "0x771000",
        "gasPrice": "0x5d21dba00",
        "hash": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
        "input": "",
        "nonce": "0x4f3",
        "senderTxHash": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
        "signatures": [
          {
            "V": "0x7f5",
            "R": "0xd63bf59c294bfd26e0f756b374af7c2b09e89a1b75bbe16d8a8999b96c97271d",
            "S": "0x64dae73afc0992b87a0a23275c00f8898681c621c6203d6a36fa427af3b0ca7e"
          }
        ],
        "to": "0x20a1cff753ae36988f73507a5497f02369970d70",
        "transactionIndex": "0x0",
        "type": "TxTypeLegacyTransaction",
        "typeInt": 0,
        "value": "0x0"
      }
    },
    {
      "contractAddress": "0xc1c88a7b90062866f54320b18ee5da65f79202d6",
      "contractName": "SiringClockAuction",
      "eventName": "AuctionCreated(uint256,uint256,uint256,uint256)",
      "data": [
        {
          "name": "tokenId",
          "value": "130",
          "type": "uint256"
        },
        {
          "name": "startingPrice",
          "value": "100000",
          "type": "uint256"
        },
        {
          "name": "endingPrice",
          "value": "100001",
          "type": "uint256"
        },
        {
          "name": "duration",
          "value": "100",
          "type": "uint256"
        }
      ],
      "transaction": {
        "blockHash": "0x6267f7e3db589571ca25ac6dfc8dd78db85215c722033dd129ee001d3d87d0b2",
        "blockNumber": "0x890ba8",
        "from": "0x1a96cdf0beb4dcd61c0fd2d5d0e852df9e631b69",
        "gas": "0x771000",
        "gasPrice": "0x5d21dba00",
        "hash": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
        "input": "",
        "nonce": "0x4f3",
        "senderTxHash": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
        "signatures": [
          {
            "V": "0x7f5",
            "R": "0xd63bf59c294bfd26e0f756b374af7c2b09e89a1b75bbe16d8a8999b96c97271d",
            "S": "0x64dae73afc0992b87a0a23275c00f8898681c621c6203d6a36fa427af3b0ca7e"
          }
        ],
        "to": "0x20a1cff753ae36988f73507a5497f02369970d70",
        "transactionIndex": "0x0",
        "type": "TxTypeLegacyTransaction",
        "typeInt": 0,
        "value": "0x0"
      }
    }
  ],
  "blockMeta": {
    "platform": "klaytn",
    "network": "baobab",
    "blockNumber": 8981416,
    "blockHeader": {
      "blockscore": "0x1",
      "extraData": "",
      "gasUsed": 140703,
      "governanceData": "0x",
      "hash": "0x6267f7e3db589571ca25ac6dfc8dd78db85215c722033dd129ee001d3d87d0b2",
      "logsBloom": "",
      "number": 8981416,
      "parentHash": "0xdb469243e1a98501c8bdf5278c9bd7c55df12fc057c561ca41f04137015d7cb6",
      "receiptsRoot": "0x82359dd8426d89949c393037c6056743401b8e1a48de6971ecc02ee2882e2e62",
      "reward": "0xa86fd667c6a340c53cc5d796ba84dbe1f29cb2f7",
      "size": 1071,
      "stateRoot": "0x87dc27fe65791184493bf4032155c32c20bae00eb79ac269ae1141ee23b135bf",
      "timestamp": 1570526845,
      "timestampFoS": "0xc",
      "totalBlockScore": "0x890ba9",
      "transactions": [
        "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d"
      ],
      "transactionsRoot": "0xd07f955010a0049e9169eac94f0312a3a9dd6ea108e01dd2efcba81af0bf836d",
      "voteData": "0x"
    }
  },
  "userMeta": {
    "integrationId": "kitty-tutorial-klaytn-uijkh",
    "userId": 1
  }
}
```

